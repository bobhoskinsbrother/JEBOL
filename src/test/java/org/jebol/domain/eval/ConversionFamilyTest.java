...
class ConversionFamilyTest {
    ...
    void aGeneratedConversionAgreesWithTheGeneralOne() {
        // Not a written-out answer, because the point is that the two
        // cannot drift apart rather than what either of them returns.
        assertThat(answerTo("""
                disagreeing: copy []
                foreach [name value] [
    ...
    void theyCarryNoTitle() {
        // The family is generated by Rebol's own mezz-types.reb now, and that
        // file decides whether to write a title by asking whether this build
        // carries them:
        ...
        //     ]
        //
        // JEBOL's natives hold a parameter list rather than a spec block, so
        // SPEC-OF rebuilds one and there is no doc string to find. The `[value]`
        // branch is taken, which is Rebol's own answer for a build like this.
    ...
    void theDegenerateSourcesConvert() {
        // Each of these is a different answer to "what is nothing here",
        // so one example would prove nothing about the others.
        // Compared in REBOL rather than as molded text, because a molded
        // answer has to survive two rounds of quoting to reach an
        // assertion here and the escaping is what breaks, not the code.
    ...
    void anEmptyWordHasNoCharactersToBeMadeOf() {
        // The one degenerate case that is an error rather than a value.
        //
        // It used to assert too-short, which was JEBOL's own answer
        // rather than R3's. The C runs the scanner over the text and
        // raises invalid-chars whenever it does not come back as one
        // word, and an empty string does not, so it is the same failure
        // that "a b" gets rather than a shortness of its own.
    ...
    void toLogicFollowsTruthiness() {
        // Zero is truthy in REBOL, so this is the case a reader coming
        // from another language gets wrong.
