...
public final class Aria {
    ...
    /** How many bytes go in and come out, whatever the key. */
    ...
    public static OneBlock under(byte[] key, boolean deciphering) {
        ...
        int[][] roundKeys = deciphering
                ? reversedForDeciphering(forwards, rounds)
                : forwards;
    ...
    private static void substitute(int[] state, int[][] tables) {
        for (int at = 0; at < WORDS; at++) {
            int word = state[at];
            state[at] = tables[0][word & 0xFF]
                    | tables[1][word >>> 8 & 0xFF] << 8
                    | tables[2][word >>> 16 & 0xFF] << 16
                    | tables[3][word >>> 24 & 0xFF] << 24;
        }
    }
    ...
    private static final int[][] ODD_ROUND_TABLES = {
        FIRST_FORWARD, SECOND_FORWARD, FIRST_BACKWARD, SECOND_BACKWARD
    };

    private static final int[][] EVEN_ROUND_TABLES = {
    ...
    private static void diffuseWhichIsItsOwnInverse(int[] state) {
        ...
        int second = swapHalves(state[3]);
        state[3] = swapWithinPairs(state[2]);
        state[2] = swapWithinPairs(second);
        carried ^= state[3];
        int third = swapHalves(state[1]);
        carried = swapWithinPairs(carried) ^ third ^ state[2];
        second ^= swapHalves(state[3]);
        third ^= swapWithinPairs(state[0]);
        state[1] ^= carried ^ second;
        second = swapHalves(second) ^ carried;
        state[0] ^= swapWithinPairs(second);
        carried = swapHalves(carried);
        state[3] ^= swapWithinPairs(carried) ^ third;
        third = swapHalves(third);
        state[2] ^= swapWithinPairs(third) ^ carried;
    }

    private static byte[] transformed(byte[] block, int[][] roundKeys, int rounds) {
        int[] state = new int[WORDS];
        for (int at = 0; at < WORDS; at++) {
            state[at] = wordAt(block, at * 4);
        }
        int round = 0;
        while (true) {
        ...
        for (int at = 0; at < WORDS; at++) {
            putWord(out, at * 4, state[at]);
        }
        return out;
    }

    private static void addTheRoundKey(int[] state, int[] roundKey) {
        for (int at = 0; at < WORDS; at++) {
            state[at] ^= roundKey[at];
        }
    }
    ...
    private static int[][] forEnciphering(byte[] key) {
        ...
        for (int at = 0; at < WORDS; at++) {
            derived[0][at] = wordAt(key, at * 4);
        }
        for (int at = 0; at * 4 + 16 < key.length; at++) {
            derived[1][at] = wordAt(key, 16 + at * 4);
        }
    ...
    private static void rotatedIntoTakenInTheOtherByteOrder(
        ...
        int carried = reversed(right[from]);
        for (int at = 0; at < WORDS; at++) {
            from = (from + 1) % 4;
            int next = reversed(right[from]);
            int shifted = carried << within;
            if (back != 0) {
                shifted |= next >>> back;
            }
            into[at] = left[at] ^ reversed(shifted);
            carried = next;
        }
    }
    ...
    private static int[][] reversedForDeciphering(int[][] forwards, int rounds) {
        ...
        for (int front = 0, back = rounds; front < back; front++, back--) {
            int[] swapped = backwards[front];
            backwards[front] = backwards[back];
    ...
    private static int wordAt(byte[] octets, int at) {
        return octets[at] & 0xFF | (octets[at + 1] & 0xFF) << 8
                | (octets[at + 2] & 0xFF) << 16 | (octets[at + 3] & 0xFF) << 24;
    }

    private static void putWord(byte[] octets, int at, int word) {
        octets[at] = (byte) word;
        octets[at + 1] = (byte) (word >>> 8);
        octets[at + 2] = (byte) (word >>> 16);
